...
const ManagePage = () => {
  ...
  const { data, isLoading, error, isError } = useHandleFindContactQuery({
    page: currentPage,
    limit,
    search: searchText,
    status,
  ...
  const [handleUpdateContactStatus, { isLoading: isUpdatingStatus }] =
    useHandleUpdateContactStatusMutation();

  const [handleUpdateContactTrash, { isLoading: isLoadingUpdateTrash }] =
    useHandleUpdateContactTrashMutation();
  ...
  const handleUpdateTrash = async (id: string) => {
    try {
      ...
      await handleUpdateContactTrash(data).unwrap();
  ...
  const handleDownloadExcel = () => {
    ...
    const url = `${baseURL}/contact/download-excel?${params}`;
  ...
  return (
    <div>
      <Filters
        ...
        headingTitle="Manage Contacts"
        headingSubtitle="Manage all your contacts. You can add, or edit them."
      />
