...
import { useHandleDeleteEventMutation, useHandleFindEventQuery, useHandleUpdateEventStatusMutation, useHandleUpdateEventTrashMutation } from "@/Redux/features/event/eventApi";
...
const ManagePage = () => {
  ...
  const { data, isLoading, error, isError } = useHandleFindEventQuery({
    page: currentPage,
    limit,
    search: searchText,
    status,
  ...
  const [handleUpdateEventStatus, { isLoading: isUpdatingStatus }] =
    useHandleUpdateEventStatusMutation();

  const [handleUpdateEventTrash, { isLoading: isLoadingUpdateTrash }] =
    useHandleUpdateEventTrashMutation();

  const [handleDeleteEvent, { isLoading: isDeleting }] =
  ...
  const handleUpdateTrash = async (id: string) => {
    try {
      ...
      await handleUpdateEventTrash(data).unwrap();
  ...
  const handleDelete = async (id: string) => {
    try {
      await handleDeleteEvent(id).unwrap();
  ...
  const handleDownloadExcel = () => {
    ...
    const url = `${baseURL}/event/download-excel?${params}`;
  ...
  return (
    <div>
      <Filters
        ...
        headingTitle="Manage Events"
        headingSubtitle="Manage all your events. You can add, or edit them."
      />
