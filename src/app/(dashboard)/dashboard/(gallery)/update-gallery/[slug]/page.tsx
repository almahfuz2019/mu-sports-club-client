...
const UpdatePage: React.FC<{ params: Promise<ParamsSlug> }> = ({ params }) => {
  ...
  const [handleUpdate, { isLoading, error }] = useHandleUpdateGalleryMutation();
  ...
  const { data } = useHandleFindSingleGalleryQuery(slug);
  useEffect(() => {
  ...
  return (
    <div>
      <Heading
        ...
        subTitle="Edit the details of an existing gallery."
      />
      <Form {...methods}>
        <form
