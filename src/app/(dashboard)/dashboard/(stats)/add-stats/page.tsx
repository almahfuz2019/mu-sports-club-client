...
import { useHandleCreateStatsMutation } from "@/Redux/features/stats/statsApi";
...
const AddPage = () => {
  ...
  const [handleCreate, { isLoading, error }] = useHandleCreateStatsMutation();
  ...
  const onSubmit: SubmitHandler<IFormInput> = async () => {
    try {
      const payload = {
        member: Number(metaData?.member),
        event: Number(metaData?.event),
        achievement: Number(metaData?.achievement),
      };
      ...
      router.push("/dashboard/manage-stats");
    } catch (error: any) {
  ...
  return (
    <div>
      <Heading
        title="Add Stats"
        subTitle="Enter information to add a new Stats to the list."
      />
      <Form {...methods}>
        <form
        ...
        >
          ...
          >
            {isLoading ? "Please Wait..." : "Add Stats"}
          </Button>
