...
const UpdatePage: React.FC<{ params: Promise<ParamsSlug> }> = ({ params }) => {
  ...
  const [handleUpdate, { isLoading, error }] = useHandleUpdateStatsMutation();
  ...
  const { data } = useHandleFindSingleStatsQuery(slug);
  useEffect(() => {
  ...
  return (
    <div>
      <Heading
        title="Update Stats"
        subTitle="Edit the details of an existing stats."
      />
      <Form {...methods}>
        <form
        ...
        >
          ...
          >
            {isLoading ? "Please Wait..." : "Update Stats"}
          </Button>
