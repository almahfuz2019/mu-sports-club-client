...
const UpdatePage: React.FC<{ params: Promise<ParamsSlug> }> = ({ params }) => {
  ...
  const [handleUpdate, { isLoading, error }] = useHandleUpdateNoticeMutation();
  ...
  const { data } = useHandleFindSingleNoticeQuery(slug);
  useEffect(() => {
  ...
  return (
    <div>
      <Heading
        title="Update Notice"
        subTitle="Edit the details of an existing notice."
      />
      <Form {...methods}>
        <form
        ...
        >
          ...
          >
            {isLoading ? "Please Wait..." : "Update Notice"}
          </Button>
