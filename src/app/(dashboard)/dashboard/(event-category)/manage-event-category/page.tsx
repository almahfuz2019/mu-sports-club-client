...
import { useHandleDeleteEventCategoryMutation, useHandleFindEventCategoryQuery, useHandleUpdateEventCategoryStatusMutation, useHandleUpdateEventCategoryTrashMutation } from "@/Redux/features/eventCategory/eventCategoryApi";
...
const ManagePage = () => {
  ...
  const { data, isLoading, error, isError } = useHandleFindEventCategoryQuery({
    page: currentPage,
    limit,
    search: searchText,
    status,
  ...
  const [handleUpdateEventCategoryStatus, { isLoading: isUpdatingStatus }] =
    useHandleUpdateEventCategoryStatusMutation();

  const [handleUpdateEventCategoryTrash, { isLoading: isLoadingUpdateTrash }] =
    useHandleUpdateEventCategoryTrashMutation();

  const [handleDeleteEventCategory, { isLoading: isDeleting }] =
  ...
  const handleStatusUpdate = async (id: string, statusForUpdate: string) => {
    try {
      await handleUpdateEventCategoryStatus({
        id,
  ...
  const handleUpdateTrash = async (id: string) => {
    try {
      ...
      await handleUpdateEventCategoryTrash(data).unwrap();
  ...
  const handleDelete = async (id: string) => {
    try {
      await handleDeleteEventCategory(id).unwrap();
  ...
  const handleDownloadExcel = () => {
    ...
    const url = `${baseURL}/event-category/download-excel?${params}`;
  ...
  return (
    <div>
      <Filters
        ...
        headingTitle="Manage Event Categories"
        headingSubtitle="Manage all your event categories. You can add, or edit them."
      />
